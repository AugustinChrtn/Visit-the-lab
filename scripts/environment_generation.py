...
class Lab_structure:
    ...
    def __init__(self):
        ...
        # General structure
        self.total_width = np.random.randint(20, 21)
        self.total_height = np.random.randint(20, 21)
        self.number_rooms_up = np.random.randint(2, 5)
        self.number_rooms_down = np.random.randint(2, 5)
        self.door_width = 2
        ...
        self.height_each_room = np.cumsum(self.height_each_room)
        self.width_rooms_up = np.cumsum(self.width_rooms_up)
        self.width_rooms_down = np.cumsum(self.width_rooms_down)

        # Walls and doors
        self.walls = np.zeros((self.total_height, self.total_width, 4))
    ...
    def list_with_variability(self, original_list, number_to_reach):
        ...
        if number_to_add > 0:
            for i in range(number_to_add):
                index_bonus = np.random.randint(len(original_list))
    ...
    def add_vertical_doors(self, height_level, threshold, number_rooms):
        ...
        for wall_number in range(number_rooms - 1):
            ...
            door_position = np.random.randint(
                height_level - self.door_width + 1) + 1
